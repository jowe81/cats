...
describe('#breedDetailsFromFile', () => {
  ...
  for (let breed in catBreeds) {
    it(`provides, via callback, breed details for the ${breed} breed`, (done) => {
      breedDetailsFromFile(breed, (dataForThisBreed) => {
        assert.strictEqual(dataForThisBreed, catBreeds[breed]);
        done();
      });
    });
  }
